...
class Snare1 : public Instrument
{
public:
	double Pos;
	double TimePos;
	...
	bool Run()
	{
		...
		CurrentFre = CurrentFre/(TimeStep*40.0+1.0);
		...
		Amp = 0;
		Amp = Amp + LowMod*SoftSqr(HPos*20.8)*TriSqr(HPos*55,0.5-Rise2(PlayPos));//*(Soft(HPos*31.5)*Soft(HPos*13.45)*0.2+0.8);
		Amp = Amp + NoiseMod*0.5*Soft(Rise2(Soft(HPos*52.56)*Soft(HPos*150)*Soft(HPos*137.8)*Soft(HPos*95.43)*Soft(HPos*113.6784)*Saw2(HPos*116.9)*8));
