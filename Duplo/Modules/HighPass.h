...
class HighPass : public Dup::Dup_Module
{
...
public:
	...
	static const dup_uint32 PATHID = 113;
	...
	HighPass(Dup::ModuleHandler *moduleH, dup_uint32 id, Dup::MessageParser msgParser) :
	Dup_Module(moduleH,
		id,
		N_INPUTS,
		N_OUTPUTS)
	...
	virtual void run()
	{
		...
		*(bufferL + head) = inputL;
		*(bufferR + head) = inputR;
		outputL = inputL - (bufferSumL/bufferSize);
		outputR = inputR - (bufferSumR/bufferSize);
